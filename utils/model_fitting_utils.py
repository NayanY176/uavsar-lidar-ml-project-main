...
from catboost import CatBoostRegressor # New: Import CatBoostRegressor
...
class ModelFitting:
    ...
    def __init__(
        ...
        **model_params
        ...
        elif self.model_name == 'catboost': # New: Handle catboost
            self.model_params = {**initial_params['catboost'], **model_params}

        else:
            raise ValueError(f'Invalid model name: {self.model_name}. \nPlease choose from: extra_trees, xgboost, pytorch_nn, linear_regression, random_forest, lightgbm, catboost.')
    ...
    def fit_model(self) -> None:
        ...
        elif self.model_name == 'catboost': # New: Fit CatBoost Regressor
    ...
    def make_predictions(self) -> Dict[str, pd.DataFrame]:
        ...
        elif self.model_name == 'catboost': # New: Make CatBoost predictions
            self.y_pred_test = self.model.predict(self.split['X_test'][self.var])
            y_pred_test_df = pd.DataFrame(
                data = self.y_pred_test,
                columns = ['snow_depth_pred']
    ...
    def evaluate_model(self) -> pd.DataFrame:
        ...
        elif self.model_name == 'catboost': # New: Evaluate CatBoost
            train_eval=compute_metrics(
                y_true=self.split['y_temp'],
                y_pred=self.y_pred_train,
                model_name=self.model_name + '_train'
    ...
    def get_importance(self) -> pd.DataFrame:
        ...
        elif self.model_name == 'catboost': # New: Get feature importance for CatBoost
            feature_importance = pd.DataFrame(
                data = {
                    'feature': self.split['X_temp'][self.var].columns,
